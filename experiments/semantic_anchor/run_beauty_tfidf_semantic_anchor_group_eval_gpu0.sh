...
eval_one() {
  ...
  local gate="${11}"
  local weight="${12}"
  local semantic_embedding_path="${13}"
  local checkpoint="${14}"
...
tail -n +2 "$TASK_FILE" | while IFS=$'\t' read -r run_name dataset method model hidden max_len inner_size temp svd_dim mode gate weight semantic_embedding_path checkpoint; do
  eval_one "$run_name" "$dataset" "$method" "$model" "$hidden" "$max_len" "$inner_size" "$temp" "$svd_dim" "$mode" "$gate" "$weight" "$semantic_embedding_path" "$checkpoint"
done
