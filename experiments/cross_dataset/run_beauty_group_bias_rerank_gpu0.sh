...
# Validation-selected group-bias reranking on top of CaNDS full-sort scores.
...
ALPHAS="${ALPHAS:-0,0.05,0.1,0.2,0.3,0.5,0.75,1.0,1.5,2.0}"
...
OUT_DIR="${OUT_DIR:-$ROOT/analysis_results/beauty_group_bias_rerank}"
LOG_DIR="${LOG_DIR:-$ROOT/log_runs/beauty_group_bias_rerank_gpu0}"
...
run_one() {
  ...
  conda run --no-capture-output -n "$CONDA_ENV" python experiments/cross_dataset/analyze_group_bias_rerank.py \
    --dataset "$DATASET" \
    --gpu_id "$GPU_ID" \
    --seed "$SEED" \
    --model "$cands_model" \
