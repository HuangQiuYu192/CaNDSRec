...
# Ablation for inverse-popularity sample weighting in angular-neighbor smoothing.
...
POP_WEIGHTS_STR="${POP_WEIGHTS_STR:-True False}"

BASE_CKPT_DIR="${BASE_CKPT_DIR:-$ROOT/ckpt/main_benchmark_grid}"
TAG="${TAG:-beauty_angular_smooth_popweight_ablation_gpu0}"
LOG_DIR="${LOG_DIR:-$ROOT/log_runs/$TAG}"
CKPT_DIR="${CKPT_DIR:-$ROOT/ckpt/$TAG}"
GROUP_OUT_DIR="${GROUP_OUT_DIR:-$ROOT/analysis_results/beauty_angular_smooth_popweight_ablation}"
...
suffix_for_pop_weight() {
  case "$1" in
    True|true|1|yes|y) echo "popwTrue" ;;
    *) echo "popwFalse" ;;
  esac
}
...
train_one() {
  local pop_weight="$1"
  local suffix
  suffix="$(suffix_for_pop_weight "$pop_weight")"
  local name="${DATASET}_AngularSmoothCANDSSASRec_h${HIDDEN_SIZE}_len${MAX_LEN}_temp${TEMPERATURE}_w${SMOOTH_WEIGHT}_k${SMOOTH_K}_st${SMOOTH_TEMP}_q${SMOOTH_QUANTILE}_thr${SIM_THRESHOLD}_${suffix}"
  local log_file="$LOG_DIR/${name}.log"
  local ckpt_path="$CKPT_DIR/${name}"

  if grep -q "test result" "$log_file" 2>/dev/null; then
    echo "[$(date '+%F %T')] SKIP train pop_weight=${pop_weight}" | tee -a "$LOG_DIR/master.log"
    return 0
  fi

  mkdir -p "$ckpt_path"
  echo "[$(date '+%F %T')] START train pop_weight=${pop_weight} gpu=$GPU_ID" | tee -a "$LOG_DIR/master.log"
  ...
  echo "[$(date '+%F %T')] DONE train pop_weight=${pop_weight}" | tee -a "$LOG_DIR/master.log"
}
...
echo "[$(date '+%F %T')] pop_weights=$POP_WEIGHTS_STR gpu=$GPU_ID" | tee -a "$LOG_DIR/master.log"

for pop_weight in $POP_WEIGHTS_STR; do
  train_one "$pop_weight"
done
...
group_eval_one "CaNDS_h${HIDDEN_SIZE}_temp${TEMPERATURE}" CANDSSASRec "$base_ckpt" False

for pop_weight in $POP_WEIGHTS_STR; do
  suffix="$(suffix_for_pop_weight "$pop_weight")"
  run_name="${DATASET}_AngularSmoothCANDSSASRec_h${HIDDEN_SIZE}_len${MAX_LEN}_temp${TEMPERATURE}_w${SMOOTH_WEIGHT}_k${SMOOTH_K}_st${SMOOTH_TEMP}_q${SMOOTH_QUANTILE}_thr${SIM_THRESHOLD}_${suffix}"
  ckpt="$(latest_checkpoint_in_dir "$CKPT_DIR" "$run_name")"
  tag="AS_${suffix}"
  group_eval_one "$tag" AngularSmoothCANDSSASRec "$ckpt" "$pop_weight"
done
...
order = {"CaNDS_h256_temp10": 0, "AS_popwTrue": 1, "AS_popwFalse": 2}
