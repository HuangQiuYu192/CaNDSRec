...
# Validation-only temperature sensitivity. Runs serially on GPU 1 so it never
# overlaps another task on that card. The test set is only read by main.py's
...
for dataset in $DATASETS_STR; do
  for seed in $SEEDS_STR; do
    for temp in $TEMPS_STR; do
      ...
      echo "START dataset=$dataset seed=$seed temp=$temp (GPU 1)"
      conda run --no-capture-output -n "$CONDA_ENV" python main.py \
        --model CANDSSASRec --dataset "$dataset" --gpu_id 1 --seed "$seed" \
