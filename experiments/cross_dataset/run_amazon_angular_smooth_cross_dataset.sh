...
# Cross-dataset validation for AngularSmoothCANDSSASRec on Amazon-style sparse datasets.
# Default: Beauty/Sports/Toys, hidden size 256, temperature 10, GPUs 0 and 1.
...
TAG="${TAG:-amazon_angular_smooth_cross_dataset}"
LOG_DIR="${LOG_DIR:-$ROOT/log_runs/$TAG}"
CKPT_DIR="${CKPT_DIR:-$ROOT/ckpt/$TAG}"
GROUP_OUT_DIR="${GROUP_OUT_DIR:-$ROOT/analysis_results/amazon_angular_smooth_cross_dataset}"
...
for dataset in $DATASETS_STR; do
  ...
  group_eval_one "${dataset}_CaNDS_h${HIDDEN_SIZE}_temp${TEMPERATURE}" "$dataset" CANDSSASRec "$base_ckpt"
...
model_order = {"CANDSSASRec": 0, "AngularSmoothCANDSSASRec": 1}
...
preferred = [
    "dataset", "tag", "model", "hidden", "temperature", "angular_smooth_weight",
    "angular_smooth_k", "angular_smooth_temperature", "angular_smooth_pop_quantile",
    "angular_smooth_sim_threshold", "group", "n", "median_rank",
