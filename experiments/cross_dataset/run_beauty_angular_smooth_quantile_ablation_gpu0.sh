...
# Ablation for which targets receive angular-neighbor smoothing.
# q=0.33: tail-focused, q=0.67: mid+tail, q=1.0: all active items.
...
QUANTILES_STR="${QUANTILES_STR:-0.33 0.67 1.0}"

BASE_CKPT_DIR="${BASE_CKPT_DIR:-$ROOT/ckpt/main_benchmark_grid}"
TAG="${TAG:-beauty_angular_smooth_quantile_ablation_gpu0}"
LOG_DIR="${LOG_DIR:-$ROOT/log_runs/$TAG}"
CKPT_DIR="${CKPT_DIR:-$ROOT/ckpt/$TAG}"
GROUP_OUT_DIR="${GROUP_OUT_DIR:-$ROOT/analysis_results/beauty_angular_smooth_quantile_ablation}"
...
train_one() {
  local quantile="$1"
  local name="${DATASET}_AngularSmoothCANDSSASRec_h${HIDDEN_SIZE}_len${MAX_LEN}_temp${TEMPERATURE}_w${SMOOTH_WEIGHT}_k${SMOOTH_K}_st${SMOOTH_TEMP}_q${quantile}_thr${SIM_THRESHOLD}"
  local log_file="$LOG_DIR/${name}.log"
  local ckpt_path="$CKPT_DIR/${name}"

  if grep -q "test result" "$log_file" 2>/dev/null; then
    echo "[$(date '+%F %T')] SKIP train q=${quantile}" | tee -a "$LOG_DIR/master.log"
    return 0
  fi

  mkdir -p "$ckpt_path"
  echo "[$(date '+%F %T')] START train q=${quantile} gpu=$GPU_ID" | tee -a "$LOG_DIR/master.log"
  ...
  echo "[$(date '+%F %T')] DONE train q=${quantile}" | tee -a "$LOG_DIR/master.log"
}
...
echo "[$(date '+%F %T')] quantiles=$QUANTILES_STR gpu=$GPU_ID" | tee -a "$LOG_DIR/master.log"

for quantile in $QUANTILES_STR; do
  train_one "$quantile"
done
...
group_eval_one "CaNDS_h${HIDDEN_SIZE}_temp${TEMPERATURE}" CANDSSASRec "$base_ckpt" 0.67

for quantile in $QUANTILES_STR; do
  run_name="${DATASET}_AngularSmoothCANDSSASRec_h${HIDDEN_SIZE}_len${MAX_LEN}_temp${TEMPERATURE}_w${SMOOTH_WEIGHT}_k${SMOOTH_K}_st${SMOOTH_TEMP}_q${quantile}_thr${SIM_THRESHOLD}"
  ckpt="$(latest_checkpoint_in_dir "$CKPT_DIR" "$run_name")"
  tag="AS_q${quantile}"
  group_eval_one "$tag" AngularSmoothCANDSSASRec "$ckpt" "$quantile"
done
...
order = {"CaNDS_h256_temp10": 0, "AS_q0.33": 1, "AS_q0.67": 2, "AS_q1.0": 3}
