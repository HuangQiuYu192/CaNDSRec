...
OUT="$ROOT/log_runs/lastfm_s3rec_temp_grid_h64"
mkdir -p "$OUT"
cd "$ROOT"

source "$ENV"

for temp in 5.0 7.5 15.0 30.0 40.0; do
  tag="LastFM-S3Rec_CANDSSASRec_h64_len50_temp${temp}"
  log="$OUT/${tag}.log"
  echo "[$(date '+%F %T')] START $tag gpu=1" | tee -a "$OUT/master.log"
