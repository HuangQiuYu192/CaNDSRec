#!/usr/bin/env python3
"""Mechanism diagnostics for SASRec vs CANDSSASRec."""
...
def main():
    ...
    _, _, _, test_data_cands, cands = build_model("CANDSSASRec", args.cands_checkpoint, args)
    ...
    dot_stats = collect_rank_stats(sasrec, test_data, "dot", args.max_batches)
    ...
    for model_name, model, stats in [
        ("SASRec", sasrec, dot_stats),
        ("CANDSSASRec", cands, cands_stats),
    ]:
    ...
    for group, idx in groups.items():
        row = dict(base)
        row["model"] = "CANDSSASRec_minus_SASRec"
        row["group"] = group
        row["n"] = int(len(idx))
