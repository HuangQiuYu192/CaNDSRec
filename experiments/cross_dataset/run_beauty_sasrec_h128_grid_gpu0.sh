...
# Beauty-only SASRec h128 benchmark.
# Runs SASRec dot-product baseline and CANDSSASRec cosine-temperature grid.
...
TAG="${TAG:-beauty_sasrec_h128_grid_gpu0}"
...
write_task() {
  ...
  printf "%s\t%s\t%s\n" "$name" "$model" "$temperature" >> "$TASK_FILE"
}

build_tasks() {
  : > "$TASK_FILE"
  write_task SASRec 10.0
  for temp in $TEMPS_STR; do
    write_task CANDSSASRec "$temp"
  done
}
...
while IFS=$'\t' read -r name model temperature; do
  run_task "$name" "$model" "$temperature"
done < "$TASK_FILE"
