...
# Beauty-only fixed-inner-size control experiment.
# Varies hidden_size while keeping inner_size fixed to test whether CaNDS gains
# are tied to representation/scoring dimensionality rather than FFN capacity.
...
FIXED_INNER_SIZE="${FIXED_INNER_SIZE:-256}"
...
TAG="${TAG:-beauty_sasrec_fixed_inner${FIXED_INNER_SIZE}_grid_gpu0}"
...
build_tasks() {
  ...
  for hidden_size in $HIDDEN_SIZES_STR; do
    write_task "$hidden_size" SASRec 10.0
    for temp in $TEMPS_STR; do
      write_task "$hidden_size" CANDSSASRec "$temp"
    done
  done
}
...
run_task() {
  ...
  mkdir -p "$ckpt_path"
  echo "[$(date '+%F %T')] START $name inner=$FIXED_INNER_SIZE gpu=$GPU_ID" | tee -a "$LOG_DIR/master.log"
...
echo "[$(date '+%F %T')] fixed_inner_size=$FIXED_INNER_SIZE hidden_sizes=$HIDDEN_SIZES_STR gpu=$GPU_ID" | tee -a "$LOG_DIR/master.log"
echo "[$(date '+%F %T')] tasks=$(wc -l < "$TASK_FILE")" | tee -a "$LOG_DIR/master.log"

while IFS=$'\t' read -r name hidden_size model temperature; do
  run_task "$name" "$hidden_size" "$model" "$temperature"
done < "$TASK_FILE"

python experiments/cross_dataset/collect_main_benchmark_results.py \
  --log_dir "$LOG_DIR" \
  --out_csv "$LOG_DIR/summary.csv" \
  --out_md "$LOG_DIR/summary.md"
