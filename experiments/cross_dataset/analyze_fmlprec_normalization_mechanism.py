#!/usr/bin/env python3
"""Mechanism diagnostics for FMLPRec vs CANDSFMLPRec."""
...
def main():
    ...
    parser.add_argument("--fmlprec_checkpoint", required=True)
    ...
    config, dataset, train_data, test_data, fmlprec = build_model(
        "FMLPRec", args.fmlprec_checkpoint, args
    )
    _, _, _, test_data_cands, cands = build_model("CANDSFMLPRec", args.cands_checkpoint, args)
    ...
    dot_stats = collect_rank_stats(fmlprec, test_data, "dot", args.max_batches)
    ...
    base = {
        ...
        "fmlprec_checkpoint": args.fmlprec_checkpoint,
        "cands_checkpoint": args.cands_checkpoint,
    }

    for model_name, model, stats in [
        ("FMLPRec", fmlprec, dot_stats),
        ("CANDSFMLPRec", cands, cands_stats),
    ]:
    ...
    for group, idx in groups.items():
        row = dict(base)
        row["model"] = "CANDSFMLPRec_minus_FMLPRec"
        row["group"] = group
        row["n"] = int(len(idx))
