...
def build_transition_evidence(train_data, model, config, eligible_tail: set[int]):
    ...
    return edge_counts, target_contexts, target_predecessors


def critic_candidates(history, edge_counts, target_contexts, target_predecessors, decay, max_candidates):
    ...
    for target, weighted_count in aggregate.items():
        ...
        rows.append((int(target), float(score), float(weighted_count), int(max_edge[target]), int(target_contexts[target]), int(len(target_predecessors[target]))))
    rows.sort(key=lambda row: (-row[1], -row[2], row[0]))
    return rows[:max_candidates]


def collect_split(data, model, edge_counts, target_contexts, target_predecessors, eligible_tail, tail_items, window, decay, max_candidates):
    ...
    for batched_data in data:
        ...
        for target, history in zip(targets, histories):
            if int(target) not in tail_items:
                continue
            candidates = critic_candidates(history, edge_counts, target_contexts, target_predecessors, decay, max_candidates)
            if not candidates:
                continue
...
def proposal_rows(examples, threshold):
    """Use the top critic candidate, retaining it only above the threshold."""
    rows = []
    for example in examples:
        target, score, weighted_count, max_edge, contexts, predecessors = example["candidates"][0]
...
def metric_row(split, variant, rows):
    ...
    return {
        ...
        "proposal_recall": float(np.mean([row["hit"] for row in eligible])) if eligible else math.nan,
...
def choose_threshold(valid_examples, quantiles, min_retention):
    ...
    thresholds = sorted({float(np.quantile(scores, quantile)) for quantile in quantiles} | {float(scores.min() - 1e-9)})
    trials = []
...
def write_outputs(out_prefix, selected, trials, test_metric, calibration, metadata):
    ...
    summary_rows = trials + [test_metric]
    ...
    lines += ["", "## Frozen test result", "", "| eligible contexts | retained contexts | retention | proposal precision | proposal recall |", "| ---: | ---: | ---: | ---: |", f"| {test_metric['eligible_tail_contexts']} | {test_metric['retained_contexts']} | {test_metric['retention_rate']:.4f} | {test_metric['proposal_precision']:.4f} | {test_metric['proposal_recall']:.4f} |", "", "## Interpretation", "", "- The threshold is selected only on validation. The test row is a single frozen check.", "- `proposal_precision` is exact next-item precision of the one retained continuation, so it is conservative: other plausible but unobserved items count as non-hits.", "- Proceed to pseudo-training only if retained test precision exceeds the unfiltered top-transition baseline at an acceptable retention rate. The accompanying calibration table should be monotonic or nearly so.", ""]
...
def main():
    ...
    edge_counts, target_contexts, target_predecessors = build_transition_evidence(train_data, model, config, eligible_tail)
    valid_examples = collect_split(valid_data, model, edge_counts, target_contexts, target_predecessors, eligible_tail, tail_items, args.recent_window, args.transition_decay, args.max_candidates)
    test_examples = collect_split(test_data, model, edge_counts, target_contexts, target_predecessors, eligible_tail, tail_items, args.recent_window, args.transition_decay, args.max_candidates)
    ...
    test_metric = metric_row("test", "selected_validation_threshold", test_rows)
    ...
    write_outputs(Path(args.out_prefix), selected, trials, test_metric, calibration, metadata)
