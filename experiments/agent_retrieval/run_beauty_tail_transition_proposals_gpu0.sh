...
# Stage 1: validate train-only transition proposals before any pseudo training.
ROOT="${ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
CONDA_SH="${CONDA_SH:-/home/ssh_user/miniconda3/etc/profile.d/conda.sh}"
CONDA_ENV="${CONDA_ENV:-recbole}"
GPU_ID="${GPU_ID:-0}"
OUT_DIR="${OUT_DIR:-$ROOT/analysis_results/agent_simulator/beauty_tail_transition_proposals}"
LOG_DIR="${LOG_DIR:-$ROOT/log_runs/agent_simulator/beauty_tail_transition_proposals}"
...
echo "START Stage-1 transition proposal validation checkpoint=$CHECKPOINT"
conda run --no-capture-output -n "$CONDA_ENV" python experiments/agent_retrieval/analyze_tail_transition_proposals.py \
  ...
  --cutoffs 5,10,20,50 --out_prefix "$OUT_DIR/$tag" | tee "$LOG_DIR/${tag}.log"
echo "ALL_DONE"
