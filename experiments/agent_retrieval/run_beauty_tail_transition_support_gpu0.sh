...
# Stage 0 only: audit real train-split support before any user simulation.
ROOT="${ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
CONDA_SH="${CONDA_SH:-/home/ssh_user/miniconda3/etc/profile.d/conda.sh}"
CONDA_ENV="${CONDA_ENV:-recbole}"
GPU_ID="${GPU_ID:-0}"
OUT_DIR="${OUT_DIR:-$ROOT/analysis_results/agent_simulator/beauty_tail_transition_support}"
LOG_DIR="${LOG_DIR:-$ROOT/log_runs/agent_simulator/beauty_tail_transition_support}"
MIN_CONTEXTS="${MIN_CONTEXTS:-3}"
MIN_PREDECESSORS="${MIN_PREDECESSORS:-2}"
...
echo "START Stage-0 support audit checkpoint=$CHECKPOINT"
conda run --no-capture-output -n "$CONDA_ENV" python experiments/agent_retrieval/analyze_tail_transition_support.py \
  ...
  --eval_batch_size 512 --min_contexts "$MIN_CONTEXTS" --min_predecessors "$MIN_PREDECESSORS" \
